    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-6">
                <form action="{{ route('personal-links.update', $link) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="label" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Texto del enlace') }}</label>
                        <input type="text" name="label" id="label" value="{{ old('label', $link->label) }}" required maxlength="255"
                    ...
                    <div>
                        ...
                        <input type="url" name="url" id="url" value="{{ old('url', $link->url) }}" required maxlength="2048"
                            class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('url')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $link->sort_order) }}" min="0" step="1"
