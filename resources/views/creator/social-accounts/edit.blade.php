    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Editar red social') }}
            </h2>
        </div>
    </x-slot>

    @php
        $initialNetwork = $socialNetworks->firstWhere('id', (int) old('social_network_id', $socialAccount->social_network_id));
        $initialUsername = old('username', $socialAccount->username);
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <form action="{{ route('social-accounts.update', $socialAccount) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="social_network_id" id="social_network_id" required class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @foreach ($socialNetworks as $network)
                                <option value="{{ $network->id }}" data-slug="{{ $network->slug }}" @selected(old('social_network_id', $socialAccount->social_network_id) == $network->id)>{{ $network->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="display_name" id="display_name" value="{{ old('display_name', $socialAccount->display_name) }}" maxlength="255" placeholder="{{ __('Nombre para identificar esta cuenta') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username" value="{{ old('username', $socialAccount->username) }}" required maxlength="255" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('username')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @include('creator.social-accounts.partials.status-and-block-fields', [
                        'defaultStatus' => old('current_status', $socialAccount->current_status),
                    ...
                    <div class="rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-900/40 px-4 py-3">
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            @if ($socialAccount->is_verified)
                                <span class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400"><x-feather-icon name="check-circle" class="w-4 h-4 shrink-0" />{{ __('Sí — se gestiona desde el listado de redes.') }}</span>
                            @else
                                {{ __('No — puedes marcarla como verificada en el listado de redes (acción «Marcar verificada»).') }}
                            @endif
                        </p>
                    </div>

                    @php
                        $isPrimaryValue = old('is_primary') !== null
                            ? (filter_var(old('is_primary'), FILTER_VALIDATE_BOOLEAN) ? '1' : '0')
                            : ($socialAccount->is_primary ? '1' : '0');
                    ...
                    @include('creator.social-accounts.partials.is-primary-field', [
                        ...
                        'initialNetworkId' => (string) old('social_network_id', $socialAccount->social_network_id),
                    ...
                    @include('creator.social-accounts.partials.topic-fields', [
                        'mode' => 'edit',
                        'socialAccount' => $socialAccount,
                        'selectedTopics' => $socialAccount->topics,
                    ])
