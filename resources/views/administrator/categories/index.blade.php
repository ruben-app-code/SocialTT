                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($categorias as $cat)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ $cat->id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $cat->nombre }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center gap-2 rounded-md border border-gray-200 dark:border-gray-600 shadow-sm px-3 py-1.5 text-sm font-medium" style="background-color: {{ $cat->bg ?? '#1E85FF' }}; color: {{ $cat->color ?? '#ffffff' }};">
                                            @if($cat->icon)
                                                <x-category-icon :name="$cat->icon" class="size-4 shrink-0" />
                                            @endif
                                            {{ $cat->nombre }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm space-x-2">
                                        <a href="{{ route('categories.edit', $cat) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">{{ __('Edit') }}</a>
                                        <form action="{{ route('categories.destroy', $cat) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('¿Eliminar esta categoría?') }}');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ __('No hay categorías.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
